...
import { maybeRewriteArguments } from "../src/lib/ai";
import { fetchNewsForQuestion } from "../src/lib/news";
import {
  fetchOpenMarkets,
  fetchRecentlyClosed,
  getYesNoPrices,
...
import { computeForecast, evaluateForecast } from "../src/lib/scoring";
    ...
    for (const item of news) {
      await prisma.rawRecord.create({
        data: {
          ...
          externalKey: `${market.id}:${item.url}`,
...
async function main() {
  ...
  try {
    const open = await fetchOpenMarkets(18);
    const closed = await fetchRecentlyClosed(4);
    ...
    console.log(`Done. Markets=${marketsCount}, news=${newsCount}`);
  } catch (err) {
